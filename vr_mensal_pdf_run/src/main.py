...
def read_sindicato_valor(data_dir: Path): return load_first_excel(data_dir, ["sindicat","valor","sindicatoxvalor"])

def best_days_series(df_base: pd.DataFrame, competencia: str, uf_series: pd.Series, fallback_days: int) -> pd.Series:
    if df_base is None: return pd.Series([fallback_days] * len(uf_series), index=uf_series.index)
    ...
    days_map = {}
    if month_col and "uf" in dfb.columns and month_col in df_base.columns:
        tmp = df_base.copy()
        uf_actual = next((c for c in tmp.columns if strip_accents(str(c)).lower().replace(" ", "_")=="uf"), None)
        if not uf_actual:
            tmp2 = normalize_columns(tmp)
            sind_col2 = next((c for c in tmp2.columns if "sind" in c), None)
            if sind_col2:
                tmp2["uf"] = tmp2[sind_col2].map(extract_uf); tmp = tmp2
                uf_actual = next((c for c in tmp.columns if strip_accents(str(c)).lower().replace(" ", "_")=="uf"), None)
        if uf_actual:
    ...
    elif "dias_uteis" in dfb.columns and "uf" in dfb.columns:
        tmp = dfb[["uf","dias_uteis"]].dropna(subset=["uf"])
        tmp["uf"] = tmp["uf"].astype(str).str.upper().str.strip()
        days_map = tmp.groupby("uf")["dias_uteis"].last().to_dict()
    else:
        return pd.Series([fallback_days] * len(uf_series), index=uf_series.index)
    return uf_series.map(lambda uf: days_map.get(str(uf).upper(), fallback_days))
...
def main():
    ...
    dias_series = best_days_series(base_dias, competencia, base["uf"], fallback_days).astype(float)
